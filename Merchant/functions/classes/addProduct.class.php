...
if ((isset($_SESSION['m_id']) || isset($_GET['addproducts'])) && !isset($_POST['addProduct'])) {
    ...
    if (validate_token($_POST['token'])) { //Proccess login Form
       ...
       if( isset($_FILES['image']['name']) ? $product1 = $_FILES['image']['name'] : $product1 =  null); 
        ...
        $shop_hidden_username         = $_POST['hidden_shop_username'];
        ...
        if (empty($form_errors)) {
            try {
                ...
                if ($product1 != null) {
                    ...
                    // create sql to insert into database
                $insert_productImages = "INSERT INTO products (m_id,m_user_id,cat_name,product_name,product_description,product_price,product1) VALUES (:shop_id,:shop_user_id,:category_name,:product_name,:product_description,:product_price,:product1)";

                $product_path1 = uploadProduct($shop_hidden_username);
                ...
                $statement->execute(array(':shop_id' => $shop_id, ':shop_user_id' => $shop_user_id, ':category_name' => $shop_Product_category, ':product_name' => $shop_Product_name, ':product_description' => $shop_Product_description, ':product_price' => $shop_Product_price, ':product1' => $product_path1));
